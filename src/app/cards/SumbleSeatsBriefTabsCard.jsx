...
/* ===========================================================================
 * Helpers (shared by both tab views)
 * ========================================================================= */
...
/* ===========================================================================
 * Presentational tab views (no hooks / no async — fed entirely by props, so
 * they render instantly whenever the tab is shown, regardless of mount timing)
 * ========================================================================= */

const SeatsView = ({ loading, fetching, error, syncedCount, data, onFetchLive }) => {
  if (loading) {
    return (
  ...
  if (error) {
    return (
      <Flex direction="column" gap="small">
        ...
        <Button onClick={() => onFetchLive("/api/enrichment")} variant="secondary">Try again</Button>
      </Flex>
    );
  }
    ...
    <Flex direction="column" gap="medium">
      ...
      {notLoaded && !data?.peopleError ? (
        <EmptyState title="Confirm the seat count" imageName="contacts" layout="vertical">
          ...
          <LoadingButton loading={fetching} onClick={() => onFetchLive("/api/enrichment")} variant="primary">
      ...
      {!notLoaded ? (
        <>
          ...
          <Flex direction="row" gap="small" justify="between" align="center" wrap="wrap">
            ...
            <LoadingButton loading={fetching} onClick={() => onFetchLive("/api/refresh")} variant="secondary" size="xs">
...
const BriefView = ({ loading, generating, error, data, onGenerate }) => {
  if (loading) {
    return (
      <Flex direction="column" align="center">
  ...
  if (error) {
    return (
      <Flex direction="column" gap="small">
        ...
        <Button onClick={() => onGenerate("/api/enrichment")} variant="secondary">Try again</Button>
      </Flex>
    );
  }
  if (data?.briefError) {
  ...
  // No cached brief yet → the explainer + Generate button (gated; no paid call on open).
    ...
    return (
      <Tile>
        <Flex direction="column" gap="small">
          ...
          <LoadingButton loading={generating} onClick={() => onGenerate("/api/enrichment")} variant="primary">
    ...
    <Flex direction="column" gap="small">
      <Flex direction="row" justify="between" align="center" gap="small" wrap="wrap">
        ...
        <Flex direction="row" gap="small" align="center">
          ...
          <LoadingButton loading={generating} onClick={() => onGenerate("/api/refresh")} variant="secondary" size="xs">
            Refresh · ~50 credits
          </LoadingButton>
        </Flex>
      </Flex>
...
/* ===========================================================================
 * Card: owns all state + data fetching (always mounted), renders two tabs.
 * ========================================================================= */
...
const SumbleSeatsBriefTabsCard = ({ actions }) => {
  ...
  const [peopleFetching, setPeopleFetching] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setPeopleError(err.message || "Couldn't load SDR people.");
      } finally {
  ...
  const fetchLivePeople = async (path) => {
    try {
      setPeopleFetching(true);
      setPeopleError(null);
      setPeopleData(await callPeople(path, false));
    } catch (err) {
      setPeopleError(err.message || "Fetch failed.");
    } finally {
      setPeopleFetching(false);
    }
  };
  ...
  const callBrief = async (path, cachedOnly) => {
    const resp = await hubspot.fetch(`${BACKEND_URL}${path}`, {
      method: "POST",
  ...
  const loadBrief = async (path, cachedOnly) => {
    const json = await callBrief(path, cachedOnly);
    setBriefData(json);
    if (json.briefStatus === "pending" && pollRef.current < MAX_POLLS) {
      pollRef.current += 1;
      const wait = (json.briefRetryAfter || 20) * 1000;
      setTimeout(() => loadBrief("/api/enrichment", false), wait);
    }
    return json;
  };
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setBriefError(null);
        pollRef.current = 0;
        await loadBrief("/api/enrichment", true); // cached-only — no credits on open
      } catch (err) {
        console.error("[SumbleBrief] load error:", err);
        setBriefError(err.message || "Couldn't load the brief.");
      } finally {
  ...
  const generateBrief = async (path) => {
    try {
      setBriefGenerating(true);
      setBriefError(null);
      pollRef.current = 0;
      await loadBrief(path, false);
    } catch (err) {
      setBriefError(err.message || "Generate failed.");
    } finally {
